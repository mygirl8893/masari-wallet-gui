...
public:
    enum Status {
        Status_Ok       = Masari::UnsignedTransaction::Status_Ok,
        Status_Error    = Masari::UnsignedTransaction::Status_Error,
        Status_Critical    = Masari::UnsignedTransaction::Status_Critical
    };
    Q_ENUM(Status)

    enum Priority {
        Priority_Low    = Masari::UnsignedTransaction::Priority_Low,
        Priority_Medium = Masari::UnsignedTransaction::Priority_Medium,
        Priority_High   = Masari::UnsignedTransaction::Priority_High
    };
    Q_ENUM(Priority)
...
private:
    explicit UnsignedTransaction(Masari::UnsignedTransaction * pt, Masari::Wallet *walletImpl, QObject *parent = 0);
    ~UnsignedTransaction();
private:
    friend class Wallet;
    Masari::UnsignedTransaction * m_pimpl;
    QString m_fileName;
