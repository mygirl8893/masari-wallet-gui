...
{
    // prepare command line arguments and pass to masarid
    ...
    qDebug() << "starting masarid " + m_masarid;
    ...
    // Start masarid
    bool started = m_daemon->startDetached(m_masarid, arguments);
...
{ 
    ...
    qDebug() << status;
    // `./masarid status` returns BUSY when syncing.
...
bool DaemonManager::sendCommand(const QString &cmd,bool testnet, QString &message) const
{
    ...
    p.start(m_masarid, external_cmd);
    ...
    : QObject(parent)
{
    ...
    // Platform depetent path to masarid
#ifdef Q_OS_WIN
    m_masarid = QApplication::applicationDirPath() + "/masarid.exe";
#elif defined(Q_OS_UNIX)
    m_masarid = QApplication::applicationDirPath() + "/masarid";
#endif

    if (m_masarid.length() == 0) {
