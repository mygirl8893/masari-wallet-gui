...
# Base environnement
        ...
        cd masari/utils/build_scripts
        docker build -f android32.Dockerfile -t masari-android .
        cd ..

# Build GUI

        cd android/docker
        docker build -t masari-gui-android .
        docker create -it --name masari-gui-android masari-gui-android bash

# Get the apk

        docker cp masari-gui-android:/opt/android/masari-wallet-gui/build/release/bin/bin/QtApp-debug.apk .
...
- Using ADB (Android debugger bridge) : 
  ...
  The only place where we are allowed to play is `/data/local/tmp`. So : 
        ...
        adb  push /opt/android/masari-wallet-gui/build/release/bin/bin/QtApp-debug.apk /data/local/tmp
